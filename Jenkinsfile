      ...
      try {
        ...
        stage ('Start Preview') {
          ...
          if (fileExists('_preview_url.txt')) {
            ...
            //echo "${previewUrl}"
            echo readFile('_preview_url.txt').trim()
            currentBuild.description = readFile('_preview_build_description.html').trim()

            // send email
            String projectName = sh(returnStdout: true, script: 'git config remote.origin.url | sed -e "s|.*/\\(.*\\)\\.git.*|\\1|g"').trim()
            String commitAuthor = sh(returnStdout: true, script: 'git log -n 25 --pretty=format:%ae | grep -v "nobody\\|nowhere\\|noreply" | head -n 1').trim()
            emailext subject: "Opendocs Deploy Preview - Project:[${projectName}] Branch:[${BRANCH_NAME}] Commit:[${CURRENT_COMMIT}]",
              mimeType: 'text/html',
              to: "${commitAuthor}",
              body: '${FILE,path="_preview_email_body.html"}'
          } else {
